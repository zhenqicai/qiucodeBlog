...
layui.define(["element","jquery"],function(exports){
    ...
    Tab.prototype.setNavBar = function(data) {
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                for(var j=0;j<data[i].children.length;j++){
                    if(data[i].children[j].target == "_blank"){
                        ulHtml += '<dd><a href="javascript:;" data-url="'+data[i].children[j].hrefName+'" target="'+data[i].children[j].target+'">';
                    }else{
                        ulHtml += '<dd><a href="javascript:;" data-url="'+data[i].children[j].hrefName+'">';
                    }
                ...
                if(data[i].target == "_blank"){
                    ulHtml += '<a href="javascript:;" data-url="'+data[i].hrefName+'" target="'+data[i].target+'">';
                }else{
                    ulHtml += '<a href="javascript:;" data-url="'+data[i].hrefName+'">';
                }
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                        ulHtml += '<i class="iconfont '+data[i].icon+'" data-icon="'+data[i].icon+'"></i>';
	...
	Tab.prototype.render = function() {
		...
		$.get(url,function(res){
			...
			if($(".navBar").html() == ''){
				var _this = this;
				$(".navBar").html(Tab.prototype.setNavBar(res)).height($(window).height()-245);
